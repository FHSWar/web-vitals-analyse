...
const csvFileNameArr = readDir('source-csv');
...
export const getSinglePageArr = () => {
	...
	const singlePageWithoutDateArr = truncat(csvFileNameArr);
	return dedup(singlePageWithoutDateArr);
};

export const classifyCsv = () => {
	const singlePageArr = getSinglePageArr();
	const classifiedCsv = singlePageArr
		.map((singlePage) => csvFileNameArr.filter((file) => file.startsWith(singlePage)));
